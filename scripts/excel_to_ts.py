import openpyxl, unicodedata, re, json, os
...
for sheet_name in wb.sheetnames:
    if sheet_name.lower() in SKIP_SHEETS: continue
    ws = wb[sheet_name]
    ...
    for row in data_rows:
        ...
        groups.append({'emoji': str(row[1] or '').strip(), 'color': 'blue', 'name': name,
            'members': str(row[3] or '0'), 'verified': parse_bool(row[4]), 'desc': str(row[5] or '').strip(),
            'tags': tags, 'trending': parse_bool(row[7]), 'category': category_slug,
            'link': str(row[9] or '#').strip() or '#', 'score': parse_int(row[8])})
    print(f"  {sheet_name} -> /{category_slug} ({len(valid)} grupos)")

lines = ["// AUTO-GENERADO desde data/tgonly-grupos.xlsx — no editar manualmente", ""]
...
for g in groups:
    lines.append(f"  {{ emoji: {json.dumps(g['emoji'])}, color: 'blue', name: {json.dumps(g['name'])}, members: {json.dumps(str(g['members']))}, verified: {'true' if g['verified'] else 'false'}, desc: {json.dumps(g['desc'])}, tags: {json.dumps(g['tags'])}, trending: {'true' if g['trending'] else 'false'}, category: {json.dumps(g['category'])}, link: {json.dumps(g['link'])}, score: {g['score']} }},")
lines.append("]")
