...
export async function getAllCategories(): Promise<Category[]> {
  ...
  const cats = staticCategories.map(c => ({ ...c, count: String(catMap.get(c.slug) || parseInt(c.count) || 0) }))
  const staticSlugs = new Set(staticCategories.map(c => c.slug))
  catMap.forEach((count, slug) => { if (!staticSlugs.has(slug)) cats.push({ emoji: '📁', name: slug, count: String(count), slug }) })
  ...
  for (const cc of customCats) {
    ...
    else { const existing = cats.find(c => c.slug === cc.slug); if (existing && cc.emoji !== '📁') existing.emoji = cc.emoji }
  }
  return cats
}
