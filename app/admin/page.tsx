...
export default function AdminPage() {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    setGroups(Array.isArray(data)?data:[]); setGroupsLoading(false)
  ...
  async function syncPhotos() {
    ...
    try {
      ...
      notify(d.message||'Sincronizacion completada'); fetchGroups()
  ...
  function startEdit(g:Group) { setEditingId(g.id); setEditData({name:g.name,description:g.description||'',members:g.members,link:g.link,category:g.category,tags:g.tags||[],photo_url:g.photo_url||'',score:g.score,emoji:g.emoji||'📱'}) }
  ...
  async function saveEdit() {
    ...
    const d = await res.json()
    if(d.ok){notify('Grupo actualizado');setEditingId(null);fetchGroups()}else notify('Error al guardar',false)
  }
  ...
  async function deleteGroup(id:string,name:string) {
    ...
    const d = await res.json()
    if(d.ok){notify('Grupo eliminado');setGroups(prev=>prev.filter(g=>g.id!==id))}else notify('Error',false)
  }
  ...
  if (!authed) return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',color:'#f0eff8',...f,display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      <div style={{background:'#111118',border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:40,width:'100%',maxWidth:400}}>
        ...
        <button onClick={handleLogin} disabled={loading} style={{...btnBlue,width:'100%',padding:'12px',fontSize:15}}>{loading?'Verificando...':'Entrar'}</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',color:'#f0eff8',...f}}>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'72px 24px 60px'}}>
        {message&&<div style={{position:'fixed',top:72,right:24,zIndex:200,background:message.ok?'rgba(34,197,94,0.95)':'rgba(255,95,109,0.95)',color:message.ok?'#000':'#fff',borderRadius:10,padding:'10px 20px',fontSize:14,fontWeight:600,...f}}>{message.text}</div>}
        ...
        <div style={{display:'flex',gap:6,marginBottom:28,background:'#111118',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:4,maxWidth:520}}>
          {[{key:'grupos',label:`📋 Todos (${groups.length})`},{key:'pendientes',label:`⏳ Pendientes (${pending.length})`},{key:'categorias',label:`📂 Categorias (${pendingCats.length})`}].map(t=>(
            <button key={t.key} onClick={()=>setTab(t.key as any)} style={{flex:1,padding:'9px 8px',borderRadius:9,border:'none',cursor:'pointer',fontWeight:700,fontSize:12,background:tab===t.key?'#2AABEE':'transparent',color:tab===t.key?'#000':'#8888aa',...f,whiteSpace:'nowrap'}}>{t.label}</button>
          ))}
        </div>
        ...
        {tab==='grupos'&&(<>
          <div style={{display:'flex',gap:10,marginBottom:20,flexWrap:'wrap'}}>
            ...
            <button onClick={syncPhotos} disabled={syncing} style={{...btnBlue,opacity:syncing?0.6:1}}>{syncing?'⏳ Obteniendo fotos...':'📸 Sincronizar fotos'}</button>
                ...
                {editingId===g.id?(
                  <div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                      ...
                      <div><label style={{fontSize:12,color:'#8888aa',display:'block',marginBottom:4}}>Categoria</label><select value={editData.category||''} onChange={e=>setEditData(p=>({...p,category:e.target.value}))} style={{...inp,color:'#f0eff8'}}>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
                    ...
                    <div style={{marginBottom:16}}><label style={{fontSize:12,color:'#8888aa',display:'block',marginBottom:4}}>URL foto de perfil</label><input value={editData.photo_url||''} onChange={e=>setEditData(p=>({...p,photo_url:e.target.value}))} style={inp} placeholder="https://..." /></div>
                    <div style={{display:'flex',gap:8}}><button onClick={saveEdit} style={btnGreen}>💾 Guardar</button><button onClick={()=>setEditingId(null)} style={btnGray}>Cancelar</button></div>
                  </div>
                ):(
                  <div>
                    <div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                          ...
                          {g.trending&&<span style={{fontSize:13}}>🔥</span>}
                      ...
                      <div style={{display:'flex',flexDirection:'column',gap:6,flexShrink:0}}>
                        <button onClick={()=>quickToggle(g.id,'verified',!g.verified)} style={{...g.verified?btnBlue:btnGray,padding:'5px 12px',fontSize:12}}>{g.verified?'✓ Verificado':'○ Verificar'}</button>
                        <button onClick={()=>quickToggle(g.id,'trending',!g.trending)} style={{background:g.trending?'rgba(245,166,35,0.15)':'transparent',color:g.trending?'#f5a623':'#8888aa',border:`1px solid ${g.trending?'rgba(245,166,35,0.3)':'rgba(255,255,255,0.12)'}`,borderRadius:8,padding:'5px 12px',fontSize:12,cursor:'pointer',...f}}>{g.trending?'🔥 Trending':'○ Trending'}</button>
                      </div>
                    </div>
        ...
        {/* PENDIENTES */}
        {tab==='pendientes'&&(pending.length===0?<div style={{textAlign:'center',padding:'60px 0',color:'#8888aa'}}><p style={{fontSize:40,marginBottom:12}}>🎉</p><p>No hay grupos pendientes</p></div>:
          pending.map(g=>(
            <div key={g.id} style={card}>
        ...
        {/* CATEGORIAS */}
        {tab==='categorias'&&(pendingCats.length===0?<div style={{textAlign:'center',padding:'60px 0',color:'#8888aa'}}><p style={{fontSize:40,marginBottom:12}}>🎉</p><p>No hay categorias pendientes</p></div>:
          pendingCats.map(cat=>(
