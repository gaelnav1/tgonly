    ...
    <div className="min-h-screen bg-[#f5f7fb] text-[#11182d]" style={{fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 pt-28 pb-20">
        <div className="mb-12 pt-6">
          ...
          <p className="text-[#6f7894] text-lg max-w-xl">{categories.length} categorias con miles de grupos activos para toda LATAM.</p>
        </div>
            ...
            <Link key={cat.slug} href={`/grupos/${cat.slug}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 text-[12px]">
                  <span className="text-[#1769ff] font-semibold">{cat.realCount || cat.count} grupos</span>
                  {cat.trending > 0 && <span className="text-amber-400">🔥 {cat.trending} trending</span>}
                </div>
              </div>
              <svg className="text-[#6f7894] group-hover:text-[#2AABEE] transition-colors flex-shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
            </Link>
          ))}
        </div>

        <div className="mt-16 text-center bg-white border border-[#e6eaf2] rounded-2xl p-10">
          <p className="text-4xl mb-4">📣</p>
          <h2 className="font-syne font-bold text-[22px] mb-3">¿Tu grupo no esta aqui?</h2>
          <p className="text-[#6f7894] mb-6 max-w-md mx-auto">Agrega tu grupo gratis. Lo revisamos en 24-48 horas.</p>
          <Link href="/agregar" className="inline-flex items-center gap-2 bg-[#1769ff] text-white font-bold px-8 py-3 rounded-xl hover:bg-[#0d5cef] transition-colors text-[15px]">
