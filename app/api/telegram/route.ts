  ...
  try {
    ...
    if (isPrivate) {
      ...
      try {
        ...
        return NextResponse.json({ ok: true, group: { name: nameMatch?.[1]||'', username: null, description: descMatch?.[1]||'', members: 0, photo_url: imageMatch?.[1]||null, type: 'private', link: previewUrl } })
      } catch {}
    ...
    try {
      ...
      const imageMatch = html.match(/<meta property="og:image" content="([^"]+)"/)
      const descMatch = html.match(/<meta property="og:description" content="([^"]+)"/)
      if (!nameMatch) return NextResponse.json({ error: 'Grupo no encontrado.' }, { status: 400 })
      return NextResponse.json({ ok: true, group: { name: nameMatch[1], username, description: descMatch?.[1]||'', members: 0, photo_url: imageMatch?.[1]||null, type: 'supergroup', link: `https://t.me/${username}` } })
    } catch {}
    return NextResponse.json({ error: 'No se pudo obtener info.' }, { status: 400 })
